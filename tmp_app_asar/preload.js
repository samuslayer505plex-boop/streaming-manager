...
;(function attachLogger() {
  try {
    const send = (level, args) => {
      try {
        const safe = args.map((a) => { try { return typeof a === 'string' ? a : JSON.stringify(a) } catch (e) { return String(a) } })
        ipcRenderer.send('renderer-log', { level, args: safe })
      } catch (e) {}
    }
    ;['log','info','warn','error'].forEach(m => {
      const orig = console[m]
      console[m] = (...args) => { send(m,args); try { orig.apply(console,args) } catch (e) {} }
    })
