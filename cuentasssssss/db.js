...
if (sqliteAvailable) {
  ...
  async function init() {
    ...
    // insert default users if not present (samuel, alex, paula) with passwords equal to their usernames
    try {
    ...
    // ensure 'tipo' column exists for cuentas (migration safety)
    try {
    ...
    // Helados sales and capital tables
    await run(`
  ...
  async function run(sql, params = []) {
    // support only simple INSERT/UPDATE/DELETE for cuentas via higher level calls in app
    return { lastID: null, changes: 0 }
  }

  async function all(sql, params = []) {
    const data = readJSON()
    // very naive: if querying cuentas, return them
